...
@endif
          <div class="card-body"> @if (session('status')) <div class="alert alert-success" role="alert">
              {{ session('status') }}
            </div> @endif <form method="POST" action="{{route('chuong.store')}}">
                @csrf
              <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Tiêu đề</label>
                <input id="slug" name="tieude" onkeyup="ChangeToSlug();" value="{{old('tieude')}}" type="text" class="form-control" placeholder="Tên chương..."pattern=".{5,}" title="Nhập ít nhất 5 ký tự">
              </div>

              <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Slug chương</label>
                <input id="convert_slug"  name="slug_chuong" value="{{old('slug_chuong')}}" type="text" class="form-control" placeholder="Slug chương..."pattern=".{5,}" title="Nhập ít nhất 5 ký tự">
              </div>
